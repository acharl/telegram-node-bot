...
class Scope {
  ...
  runMenu(menuData) {
    ...
    const resizeKeyboard =
      menuData.resizeKeyboard && menuData.resizeKeyboard === true
    const oneTimeKeyboard =
      menuData.oneTimeKeyboard && menuData.oneTimeKeyboard === true

    let replyMarkup = new ReplyKeyboardMarkup(
      keyboard,
      resizeKeyboard,
      oneTimeKeyboard
    )
  ...
  runForm(formData, callback) {
    ...
    const run = () => {
      ...
      this.sendMessage(formData[key].q, {
        ...
        reply_markup: formData[key].keyboard
          ? JSON.stringify({
  ...
  runInlineMenu(menuData, prevMessage) {
    ...
    if (!layout) {
      ...
      menu.forEach(item => {
        ...
        let goToNextLine = Array.isArray(layout)
          ? keyboard[line].length === layout[line]
          : keyboard[line].length === layout

        if (goToNextLine) line++
      })
    }
    ...
    if (!prevMessage) {
      this[method].apply(this, params).then(response => {
      ...
      this.api.editMessageText(menuData.message, params[0]).then(response => {
        prepareCallback(response)
      })
    }
  }
