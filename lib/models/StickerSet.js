...
class StickerSet {
   ...
   */
   constructor(name, title, isAnimated, containsMasks, stickers) {
       this._name = name
       this._title = title
   ...
   */
   ...
   static deserialize(raw) {
      return new StickerSet(
          ...
          raw['is_animated'], 
          raw['contains_masks'], 
          raw['stickers'] ? raw['stickers'].map(item => Sticker.deserialize(item)) : null
      )
   }
   ...
   serialize() {
      return { 
          ...
          is_animated: this.isAnimated ? this.isAnimated : undefined, 
